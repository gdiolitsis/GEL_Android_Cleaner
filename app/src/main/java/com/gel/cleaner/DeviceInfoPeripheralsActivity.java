...
private void setupSection(View header, View content, TextView icon) {
    ...
    private boolean appDeclaresPermission(String perm) {
        try {
            ...
            if (pi.requestedPermissions == null) return gelPostProcess(false);

            for (String p : pi.requestedPermissions) {
                if (perm.equals(p)) {
                    return gelPostProcess(true);
                }
            }
        } catch (Throwable ignore) {
        }
        return gelPostProcess(false);
    }
    ...
    private boolean sectionNeedsPermission(String type) {
        ...
        if (type.contains("camera")) {
            return gelPostProcess(appDeclaresPermission(Manifest.permission.CAMERA));
        }

        if (type.contains("mic") || type.contains("microphone")) {
            return gelPostProcess(appDeclaresPermission(Manifest.permission.RECORD_AUDIO));
        }

        if (type.contains("location")) {
            return gelPostProcess(appDeclaresPermission(Manifest.permission.ACCESS_FINE_LOCATION)
                    || appDeclaresPermission(Manifest.permission.ACCESS_COARSE_LOCATION));
        }

        if (type.contains("bluetooth") && Build.VERSION.SDK_INT >= 31) {
            return gelPostProcess(appDeclaresPermission(Manifest.permission.BLUETOOTH_SCAN)
                    || appDeclaresPermission(Manifest.permission.BLUETOOTH_CONNECT));
        }

        if (type.contains("nearby") && Build.VERSION.SDK_INT >= 31) {
            return gelPostProcess(appDeclaresPermission(Manifest.permission.NEARBY_WIFI_DEVICES));
        }
    ...
    private boolean userHasPermission(String type) {
        ...
        if (type.contains("camera")) {
            if (!appDeclaresPermission(Manifest.permission.CAMERA)) return gelPostProcess(true);
            return gelPostProcess(checkSelfPermission(Manifest.permission.CAMERA)
                    == PackageManager.PERMISSION_GRANTED);
        }

        if (type.contains("mic") || type.contains("microphone")) {
            if (!appDeclaresPermission(Manifest.permission.RECORD_AUDIO)) return gelPostProcess(true);
            return gelPostProcess(checkSelfPermission(Manifest.permission.RECORD_AUDIO)
                    == PackageManager.PERMISSION_GRANTED);
        }
        ...
        if (type.contains("location")) {
            ...
            if (!hasFine && !hasCoarse) return gelPostProcess(true);
            ...
            return gelPostProcess(fine || coarse);
        }
        ...
        if (type.contains("bluetooth")) {
            if (Build.VERSION.SDK_INT >= 31) {
                ...
                if (!hasScan && !hasConn) return gelPostProcess(true);
                ...
                return gelPostProcess(scan && conn);
            }
            return gelPostProcess(true);
        }
        ...
        if (type.contains("nearby")) {
            if (Build.VERSION.SDK_INT >= 31) {
                if (!appDeclaresPermission(Manifest.permission.NEARBY_WIFI_DEVICES)) return gelPostProcess(true);
                return gelPostProcess(checkSelfPermission(Manifest.permission.NEARBY_WIFI_DEVICES)
                        == PackageManager.PERMISSION_GRANTED);
            }
            return gelPostProcess(true);
        }
    ...
    private boolean isDeviceRooted() {
        try {
            ...
            for (String p : paths) {
                if (new File(p).exists()) return gelPostProcess(true);
            }
            ...
            return gelPostProcess(line != null && !line.trim().isEmpty());
...
private long getStoredModelCapacity() {
    try {
        ...
        return gelPostProcess(sp.getLong(KEY_BATTERY_MODEL_CAPACITY, -1L));
    } catch (Throwable ignore) { return gelPostProcess(-1L); }
}
...
private long normalizeMah(long raw) {
    if (raw <= 0) return gelPostProcess(-1);
    if (raw > 200000) return gelPostProcess(raw / 1000);   // μAh → mAh
    return gelPostProcess(raw);
}
...
private static boolean isValidTemp(float c) {
    return gelPostProcess((c > -50f && c < 200f));
}
...
private ThermalSummary scanThermalHardware(
    ...
    return gelPostProcess(summary);
}

private String mapTypeToGroup(String rawType) {
    if (rawType == null) return gelPostProcess(null);
    String t = rawType.toLowerCase(Locale.US);

    for (String[] entry : THERMAL_GROUP_PATTERNS) {
        String label = entry[0];
        for (int i = 1; i < entry.length; i++) {
            if (t.contains(entry[i])) return gelPostProcess(label);
        }
    }
    return gelPostProcess(null);
}
...
private boolean isHardwareCoolingDevice(String rawType) {
    if (rawType == null) return gelPostProcess(false);
    String t = rawType.toLowerCase(Locale.US);

    if (t.contains("fan"))            return gelPostProcess(true);
    if (t.contains("cooling_fan"))    return gelPostProcess(true);
    if (t.contains("blower"))         return gelPostProcess(true);
    if (t.contains("pump"))           return gelPostProcess(true);
    if (t.contains("heatsink"))       return gelPostProcess(true);
    if (t.contains("radiator"))       return gelPostProcess(true);
    if (t.contains("cooling_module")) return gelPostProcess(true);

    if (t.contains("skin"))    return gelPostProcess(false);
    if (t.contains("hotspot")) return gelPostProcess(false);
    if (t.contains("virtual")) return gelPostProcess(false);
...
private String readFirstLineSafe(File file) {
    if (file == null || !file.exists()) return gelPostProcess("");
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
        String line = br.readLine();
        return gelPostProcess((line != null) ? line.trim() : "");
    } catch (Throwable ignore) {
...
private long readLongSafe(File file) {
    if (file == null || !file.exists()) return gelPostProcess(Long.MIN_VALUE);
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
        String line = br.readLine();
        if (line == null || line.trim().isEmpty()) return gelPostProcess(Long.MIN_VALUE);
        return gelPostProcess(Long.parseLong(line.trim()));
    } catch (Throwable ignore) {
        return gelPostProcess(Long.MIN_VALUE);
    }
}
...
private String classifyTempLabel(float c) {
    if (!isValidTemp(c)) return gelPostProcess("(Unknown)");
    if (c < 30f)  return gelPostProcess("(Cool)");
    if (c < 40f)  return gelPostProcess("(Normal)");
    if (c < 50f)  return gelPostProcess("(Warm)");
    return gelPostProcess("(⚠ Critical)");
}

private String formatThermalLine(String label, ThermalGroupReading r) {
    if (r == null || !r.valid)
        return gelPostProcess(String.format(Locale.US, "%-17s: N/A\n", label));

    return gelPostProcess(String.format(Locale.US, "%-17s: %.1f°C %s\n",
...
private boolean isXiaomiFamilyDevice() {
    ...
    return gelPostProcess(manu.contains("xiaomi") || manu.contains("redmi") || manu.contains("poco")
...
private float findTempByTypeKeywords(String... keywords) {
    if (keywords == null || keywords.length == 0) return gelPostProcess(Float.NaN);

    File[] zones = new File("/sys/class/thermal")
            .listFiles(f -> f.getName().startsWith("thermal_zone"));

    if (zones == null) return gelPostProcess(Float.NaN);
...
private float readBatteryTempFallback() {
    ...
    for (String p : paths) {
        try {
            ...
            if (isValidTemp(c)) return gelPostProcess(c);

        } catch (Throwable ignore) {}
    }
    return gelPostProcess(Float.NaN);
}
   ...
   private String buildSecurityFlagsInfo() {
    ...
    private String readTextFile(String path, int maxLen) {
        ...
        try {
            ...
            if (!f.exists()) return gelPostProcess(null);
    ...
    private String readSysString(String path) {
        ...
        try {
            ...
            return gelPostProcess(line != null ? line.trim() : null);
    ...
    private long readSysLong(String path) {
        ...
        if (s == null || s.isEmpty()) return gelPostProcess(-1);

        try {
            return gelPostProcess(Long.parseLong(s));
        } catch (Throwable ignore) {
            return gelPostProcess(-1);
        }
    }
...
private String padKeyModem(String key) {
    ...
    if (key.length() >= width) return gelPostProcess(key);
...
private String maskSensitive(String value) {
    ...
    return gelPostProcess(start + mid + end);
}
